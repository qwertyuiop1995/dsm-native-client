...
VMM 创建、常规设置和公开生命周期采用持久结构化结果：创建只接受严格终态任务返回的稳定虚拟机 ID，模糊或取消结果只核对、不按名称认领、不重放，并保留未完全确认的任务证据；设置保存携带用户所见基线，锁内复读拒绝漂移，no-op 零写且只提交变化字段；启动与关机携带确认时状态基线。创建步骤/草稿、设置基线/草稿、确认、八类结果、计数、异常和专项刷新可跨 Activity 配置重建恢复，未提交结果可继续编辑，危险结果核对前不能离开。Android noVNC 控制台因契约未验证保持关闭。
VMM 从 NAS 已有文件创建映像使用官方 `Guest.Image.create` v1：提交前复核源文件完整基线、存储状态和名称占用，同名操作防重复且 `create` 只提交一次；随后只以 `Task.Info.get` 返回的稳定 `image_id` 核对映像名称和类型，终态核对后再 `clear`。断线或取消只回读、不重放。草稿、八类结果与任务 ID 保存在 Workspace，可跨 Activity 配置重建继续核对；这不等于跨进程持久恢复，应用进程死亡或重启后的任务 ID 恢复尚未实现。映像上传、编辑、导出及其他来源创建仍未完成。
VMM 任务中心最多读取 100 个唯一任务，界面只显示任务 token 的单向摘要稳定键、结束状态和进度，真实 token 只存在当前 Workspace 内存与请求边界。用户确认清理数量后，应用会全量复读 `list/get` 基线，只逐项清理仍为已结束且身份未漂移的任务，进行中任务不受影响；提交异常或取消仅进行一次严格回读，不自动重放，并会明确报告部分完成或结果未知。该状态可跨 Activity 配置重建，但应用进程死亡或重启后不能恢复清理基线和真实 token。
