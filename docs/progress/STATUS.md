...
| Android 客户端 | 已实现，需设备验收 | 第 83 批一次补齐三项现有只读能力：套件列表仅在 DSM 明确返回 `upgrade` 时显示更新提示；`Package.Thumb.get` v1 在运行时能力、2 MiB 响应、位图签名/解码和 4 MiB 内存 LRU 门禁下显示真实套件图标；Registry 搜索结果和详情仅以服务端 `is_official=true` 显示官方来源。套件安装/升级、Container 写入与 VMM 高级写入继续关闭。A0–A8 当前仍为 183/202（90.6%），剩余 19 项；原目标没有删除、拆分或降级。实体机、TalkBack、OEM、真实进程死亡、系统文件授权和真实 NAS 权限/字段/副作用保持未验证。 |
...
- **Download Station 待补齐**：macOS 已接入任务文件上传和官方基础设置；Android 已接入任务文件/Tracker/Peer 详情、RSS 浏览与官方单站点刷新、BT 搜索提供方/类别/标题过滤/排序选项、当前标准/eMule 聚合活动摘要，以及搜索结果经可写目录选择直接创建；官方指南未公开 RSS 完整编辑或文件优先级写参数，相关能力与 BT 协议高级设置、监听目录、NZB 服务器、RSS 和通知设置仍待版本化契约
- **Virtual Machine Manager 待补齐**：macOS 已接入基础创建、常规修改、映像删除、网络修改/删除、保护分类读取、日志和独立可全屏远程控制台；Android 已用官方公开 v1 契约接入基础创建、常规修改、公开生命周期、映像删除、任务中心只读列表与已结束任务受保护清理，并完成严格 task 归属、用户基线、状态基线、持久八态反馈和配置重建；内部网络写在行为验证前保持关闭，独立 noVNC 控制台仍因稳定契约未验证而关闭；虚拟盘/多网卡高级编辑、迁移、克隆、导入导出、快照恢复、保护计划写操作、任务持久恢复及更高级任务管理仍待实现
