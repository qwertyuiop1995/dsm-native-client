...
| Android 客户端 | 已实现，需设备验收 | 第 87–88 批已将不透明本机对象深链扩展到 Files、Photos、Chat、Download 与 VMM Guest 七类安全可精确重读对象；VMM Guest 使用官方 `Guest.get` v1、独立只读页和强类型返回栈，外链不能覆盖活跃写流程且不展示内部 ID。未验证的 Container/VMM 私有写入继续关闭。A0–A8 当前仍为 183/202（90.6%），剩余 19 项；原目标没有删除、拆分或降级。实体机、TalkBack、OEM、真实进程死亡、系统文件授权、媒体格式矩阵和真实 NAS 权限/字段/副作用保持未验证。 |
