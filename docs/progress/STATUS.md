...
| Android 客户端 | 已实现，需设备验收 | 第 85 批补齐 VMM 强制关机入口与危险确认、Container v1/认证/隐私边界、下载和照片备份同步持久化、超大照片来源暂停反馈、EXIF 方向及全屏预览安全区；未验证的 Container/VMM 私有写入继续关闭。A0–A8 当前仍为 183/202（90.6%），剩余 19 项；原目标没有删除、拆分或降级。实体机、TalkBack、OEM、真实进程死亡、系统文件授权、媒体格式矩阵和真实 NAS 权限/字段/副作用保持未验证。 |
