...
- macOS 工作区的详情根视图必须占满 `NavigationSplitView` 提供的可用区域，带标题栏、筛选栏或列表的页面使用顶部对齐。
- 页面没有内容时必须显示图标、原因和下一步操作，不能返回无尺寸的空视图或依赖父容器猜测位置。
- SwiftUI 页面级内容统一使用 `fillsAvailableContentArea(alignment:)`。普通空状态保持居中，标题栏和筛选栏保持 `.topLeading`。
- 新模块即使当前只有少量内容，也不得只依赖视图的固有高度；必须分别检查加载、空内容、筛选后为空、错误和正常列表五种状态。
- 弹窗中的加载、错误和空内容分支必须占满标题栏以下的剩余区域，避免状态切换时位置跳动。
