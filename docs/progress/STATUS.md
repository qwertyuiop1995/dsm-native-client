...
| Synology Chat 模块 | 进行中 | 当前 Chat Server 官方客户端已确认附件上传契约；macOS 已接入单附件上传、进度、取消和重试，等待新构建实际送达验收；首次单聊、贴纸资源、下载预览、语音、投票和加密未完成 |
...
- **Synology Chat**：已建立普通用户聊天契约与独立内部适配器；公开 `SYNO.Chat.External` 不用于普通用户聊天。第一批 `SYNO.Chat.*` 能力已按运行时发现接入，图片/视频/文件上传已按 Chat Server 2.4.1-22111 官方客户端契约实现，真实送达待验收；附件下载预览、语音、投票和加密仍待实现
...
- Apple 共享包完整测试：146 项执行通过，1 项需要真实环境的可选测试跳过。
...
| 里程碑 | 状态 | 下一出口 |
| --- | --- | --- |
| CH0 协议与会话探测 | 进行中 | 已确认 Chat Server 2.4.1-22111 官方客户端的文件上传、附件读取和内置贴纸契约；不得保存账号、地址或令牌。下一步记录脱敏 DSM build，并确认首次单聊、附件实际送达和下载参数 |
| CH1 macOS 一对一文字聊天 | 需要验证 | 已增加用户搜索、发送者目录回填、真实头像回退、双向气泡、完整时间、系统表情入口、向上分页与位置恢复、会话独立草稿、发送中/失败/重试、重试前结果复查、前台轻量增量刷新、新消息提示，以及本人消息/会话的单选多选右键删除与确认；删除成功改用悬浮提示，删除和关闭内部方法、管理员策略及首次匿名会话仍待实机确认 |
| CH2 macOS 私人群聊 | 需要验证 | 已接入命名私人群聊创建、加入、邀请、重复提交保护和成员复查；下一出口是至少三个专用测试账号完成建群和多人收发验证 |
| CH3 macOS 媒体、文件和语音 | 进行中（附件发送已接线） | 图片、视频和文件可单个选择并通过 `Chat.Post.create` v5 上传，支持进度、取消、失败重试和附件响应展示；146 项测试通过。下一出口是新构建实际送达、缩略图/下载/打开、批量附件和语音消息 |
...
Chat 模块按[Synology Chat 原生聊天功能开发计划](../development/NATIVE_DSM_CHAT_DEVELOPMENT_PLAN_ZH.md)推进。基础契约、Apple 领域模型、能力保护适配器、macOS 会话列表、聊天页、新建聊天、文字和单附件发送已经建立；每项内部能力仍须按 DSM build 与 Chat Server 版本分别验收，不能把附件契约外推到首次单聊、语音、投票或加密。
