...
状态：进行中。共同 Schema、Apple Chat 领域模型、macOS 界面和真实 DSM 会话适配器已实现；首次单聊、历史分页、文字、私人群聊、提醒设置、无附件投票创建和单附件上传已接线。当前 DSM/Chat Server 的首次单聊、附件、提醒、投票和实时通道静态契约已脱敏记录，等待新构建完成写入验收。
