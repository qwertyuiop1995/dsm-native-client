...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 未记录 | 2.4.1-22111 | macOS | 官方网页客户端已登录；岚仓待复验 | 已有会话可见；创建待复验 | 静态契约已确认；岚仓待复验 | 上传契约已确认；实际送达待新构建验收 | 未验证 | 未验证 | 未验证 | 2026-07-22 |
...
| 附件读取/缩略图 | `SYNO.Chat.Post.File.get/thumbnail` v2（内部接口） | 已加入能力发现，下载、缩略图和打开尚未实现 | 确认参数、响应类型、Range、权限、缓存和离线行为后再开放 |
...
| 语音、投票、加密 | 待逐项探测 | 保持关闭；附件上传的确认不代表这些能力已验证 | 完成脱敏契约、权限、幂等、结果复查和安全边界后再启用 |
