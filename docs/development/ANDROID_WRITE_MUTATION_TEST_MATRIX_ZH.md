...
- 现有测试已经形成闭环的主要族包括：文件夹创建/重命名/删除、移动、上传与文本保存、
  共享链接、下载控制与设置、Chat、网卡与多数组合设置、连接断开、套件启停和硬盘检测。
- 固定失败关闭并有零请求证据的入口是 5 项容器写入口和 2 项 VMM 内部网络写入口。
- 尚有 14 个入口族存在真实缺口，主要是解压/恢复任务取消、复制断线、账号/群组删除取消、
  RSS 刷新取消、回收站恢复取消、任务文件创建、套件卸载、部分 VMM 和电源/硬件设置。
  因此本矩阵目前是“完整盘点、未完成测试闭环”，不能据此勾选 A1 总目标。
...
<!-- WRITE-MUTATION methods=copyResult;state=pending;multi=yes;pre=PhotoMoveRepositoryTest.kt::批量复制拒绝覆盖;success=PhotoMoveRepositoryTest.kt::批量复制拒绝覆盖并复查每个目标;disconnect=gap;readback=gap;cancel=gap;partial=gap -->
...
<!-- WRITE-MUTATION methods=extractResult;state=pending;multi=yes;pre=ArchiveRepositoryTest.kt::解压目标目录只读时提交前拒绝;success=ArchiveRepositoryTest.kt::解压先读取内容拒绝覆盖并按顶层类型复查;disconnect=ArchiveRepositoryTest.kt::解压启动断线但全部目标回读存在;readback=ArchiveRepositoryTest.kt::解压完成后输出类型必须;cancel=gap;partial=ArchiveRepositoryTest.kt::解压只确认部分目标时返回部分成功 -->
<!-- WRITE-MUTATION methods=resumeArchiveMutationResult;state=pending;multi=no;pre=na;success=ArchiveRepositoryTest.kt::恢复已提交压缩任务只读轮询;disconnect=na;readback=ArchiveRepositoryTest.kt::恢复任务即使状态完成也必须输出回读成功;cancel=gap;partial=na -->
<!-- WRITE-MUTATION methods=restoreFromRecycleResult;state=pending;multi=no;pre=PhotoRestoreRepositoryTest.kt::原目录只读时提交前返回权限不足;success=PhotoRestoreRepositoryTest.kt::恢复前检查原目录权限和冲突并在移动后复查;disconnect=PhotoRestoreRepositoryTest.kt::恢复提交断线保持未确认;readback=gap;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=createDownloadFromFileResult;state=pending;multi=no;pre=DownloadCreationResultTest.kt::非法任务文件在上传前失败;success=gap;disconnect=DownloadCreationResultTest.kt::任务文件提交断线只回读;readback=gap;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=refreshDownloadRssSiteResult;state=pending;multi=no;pre=DownloadDiscoveryRepositoryTest.kt::RSS 目标已变化时不提交刷新;success=DownloadDiscoveryRepositoryTest.kt::RSS 刷新固定使用公开 v1;disconnect=DownloadDiscoveryRepositoryTest.kt::RSS 刷新提交时断线;readback=gap;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=testDdnsResult,saveDdnsResult,deleteDdnsResult,refreshDdnsResult;state=pending;multi=no;pre=DdnsMutationResultTest.kt::Provider 或 Record 不含 v1 时四入口均零网络;success=DdnsMutationResultTest.kt::新建只提交一次并严格回读;disconnect=DdnsMutationResultTest.kt::新建响应丢失后只读取一次目录;readback=DdnsMutationResultTest.kt::模糊保存单次回读失败;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=saveHardwareSettingsResult;state=pending;multi=yes;pre=HardwareSettingsMutationResultTest.kt::完整硬件基线漂移时零写入;success=NasServiceSettingsMutationTest.kt::硬件六组设置按契约提交并整体回读;disconnect=NasServiceSettingsMutationTest.kt::硬件设置中途断线停止后续写入;readback=gap;cancel=gap;partial=NasServiceSettingsMutationTest.kt::硬件设置中途断线停止后续写入并回读部分成功 -->
<!-- WRITE-MUTATION methods=performPowerActionResult;state=pending;multi=no;pre=NasServiceSettingsMutationTest.kt::电源动作能力缺失与预检拒绝均零写入;success=gap;disconnect=NasServiceSettingsMutationTest.kt::电源请求提交断线报告未确认;readback=na;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=deleteAccountResult,deleteGroupResult;state=pending;multi=no;pre=DirectoryDeletionMutationTest.kt::目录未明确允许删除时不发送写请求;success=DirectoryDeletionMutationTest.kt::账号删除使用名称数组并在目录消失后确认成功;disconnect=DirectoryDeletionMutationTest.kt::账号删除响应丢失后只回读;readback=DirectoryDeletionMutationTest.kt::账号删除后回读根缺失;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=uninstallPackageResult;state=pending;multi=no;pre=PackageUninstallMutationTest.kt::系统或未明确允许卸载的套件;success=PackageUninstallMutationTest.kt::卸载写请求断线后只回读且目标消失;disconnect=PackageUninstallMutationTest.kt::卸载写请求断线后只回读;readback=PackageUninstallMutationTest.kt::卸载后畸形回读;cancel=gap;partial=na -->
...
<!-- WRITE-MUTATION methods=updateVirtualMachineSettingsResult;state=pending;multi=yes;pre=VirtualMachineCreationRepositoryTest.kt::常规设置锁内发现用户所见基线漂移;success=VirtualMachineCreationRepositoryTest.kt::公开 VMM 常规设置提交后完整回读;disconnect=gap;readback=gap;cancel=gap;partial=gap -->
<!-- WRITE-MUTATION methods=controlVirtualMachineResult,deleteVirtualMachineResult;state=pending;multi=no;pre=VirtualMachineMutationResultTest.kt::生命周期锁内状态偏离用户所见基线;success=VirtualMachineMutationResultTest.kt::正常关机必须在列表回读为停止后才确认成功;disconnect=gap;readback=gap;cancel=gap;partial=na -->
<!-- WRITE-MUTATION methods=deleteVirtualMachineImageResult;state=pending;multi=no;pre=VirtualMachineMutationResultTest.kt::仅有内部映像 API 时删除零请求关闭;success=gap;disconnect=gap;readback=VirtualMachineMutationResultTest.kt::映像删除提交后回读断线;cancel=gap;partial=na -->
