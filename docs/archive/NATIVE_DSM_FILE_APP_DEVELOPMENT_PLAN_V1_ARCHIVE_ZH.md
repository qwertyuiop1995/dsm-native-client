...
> 归档说明：本文保留 2026-07-16 第一阶段的完整工程规格和当时的产品取舍，仅用于追溯历史，不再代表当前实现。当前有效计划请参见[当前开发与验收入口](../development/NATIVE_DSM_FILE_APP_DEVELOPMENT_PLAN_ZH.md)。当仍有效的架构、API、安全和验收规则全部迁入对应事实文档，活动引用清除且建立可追溯版本标签后，可以从当前文档树移除本归档全文。
...
- `.ts` 等含糊扩展名必须通过受认证的 Range 请求读取少量文件头并按文件签名识别；三端不得用文件大小猜测视频或文本。
- 压缩包预读、密码需求判断和文件名 `codepage` 选择必须由共享 Repository 契约提供；三端不得在界面层自行猜测或保存密码。
- 日志脱敏规则和危险操作确认语义。
...
| 压缩包预读与解压 | `SYNO.FileStation.Extract.list/start/status/stop` | 官方 | 密码检测、文件名编码选择、解压和进度 |
...
公开的 `CheckPermission.write` 只承诺检查目标目录中新建项目的权限。覆盖上传应使用一次性文件名执行 `create_only=true` 检查，避免部分 DSM 对“已存在文件名 + create_only=false”返回未公开错误；随后由 `Upload.upload(overwrite=true)` 完成最终权限判断，并在上传成功后读取目标文件校验结果。
