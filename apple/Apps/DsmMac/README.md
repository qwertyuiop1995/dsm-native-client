...
- 主 App 的 SID、SynoToken 和可选密码写入应用沙盒内的 AES-GCM 加密文件。只有用户创建 Finder 云盘映射后，最小必要会话才会共享到系统钥匙串，密码不会写入共享钥匙串。
...
确认设置后，脚本会生成 `dist/LanStash.app` 和 `dist/LanStash-<版本>-<架构>.dmg`。每一步直接按回车即可使用推荐选项，输入 `q` 可以随时退出。构建前会显示当前分支和提交，并检查 `Sources` 中的 Swift 文件是否全部加入 App 构建目标；产物的 `Info.plist` 会记录 `LanStashSourceCommit`，便于确认安装包对应的源码版本。选择打包后运行时会启动新实例，避免仍在运行的旧版本被误认为新产物。
