...
后台备份、Chat 会话创建以及 VMM 映像创建的真实格式、权限和写入结果仍待设备与真实 NAS 复验；Chat 内部写能力只在运行时能力覆盖已记录版本时开放，其他能力继续按兼容证据分批接入。
