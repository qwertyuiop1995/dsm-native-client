...
> 最后更新：2026-07-19
> 当前状态：PH0 进行中；PH1 已实现且文件夹扫描已获实机确认；PH2/PH3 基础能力已实现，等待完整验收
> 首个参考平台：macOS
...
- iPhone、iPad 和 Android 系统照片库授权与增量扫描。
- 仅 Wi-Fi、充电时、是否包含视频/RAW/截图等备份规则。
- 后台上传、断点任务恢复、失败重试、同名处理和重复检测。
...
| PH6 iPhone/iPad 与自动备份 | 3–5 周 | 触控原生照片 UI、系统照片库、后台备份、任务恢复和空间释放保护 | 前后台切换、权限变化、网络切换、重复项和原件核对场景通过 |
...
PH0–PH5 是 macOS 照片管理首轮，建议投入约 9–12 个有效开发周。内部接口兼容问题不能通过压缩验收时间解决；PH4 可延后发布，不得阻塞 PH1–PH3 的基础照片库。
...
2. 补充分辨率、相机、镜头、位置等 EXIF 详情，以及时间线年/月快速定位和 1 千/1 万/10 万项目性能验证。
3. 照片页尚未提供移动、收藏、基础相册入口和回收站恢复入口；恢复能力仍可在文件浏览器中使用，并受兼容开关保护。
4. PH4 智能相册、人物、主题、地点与标签仅在真实版本证据充分时启用，不以内部接口阻塞基础照片库。
5. iPhone/iPad 自动备份、Android 和 Windows 仍按 PH6–PH8 推进。
