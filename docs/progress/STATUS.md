...
| Synology Chat 模块 | 进行中 | 当前 Chat Server 已确认首次单聊、附件、提醒、投票和实时通道契约；macOS 已接入首次单聊与无附件投票创建，等待新构建写入验收；下载预览、语音、投票参与、实时通道和加密未完成 |
...
- **Synology Chat**：已建立普通用户聊天契约与独立内部适配器；公开 `SYNO.Chat.External` 不用于普通用户聊天。第一批 `SYNO.Chat.*` 能力已按运行时发现接入，首次单聊、无附件投票创建及图片/视频/文件上传已按 Chat Server 2.4.1-22111 官方客户端契约实现，真实写入待验收；附件下载预览、语音、投票参与、实时通道和加密仍待实现
...
- Apple 共享包完整测试：149 项执行通过，1 项需要真实环境的可选测试跳过。
...
| CH3 macOS 媒体、文件和语音 | 进行中（附件发送已接线） | 图片、视频和文件可单个选择并通过 `Chat.Post.create` v5 上传，支持进度、取消、失败重试和附件响应展示；完整 Apple 测试 149 项执行、0 失败。下一出口是实际送达、缩略图/下载/打开、批量附件和语音消息 |
| CH4 macOS 提醒与投票 | 进行中 | 提醒设置已接入；无附件投票创建、校验、去重、结果复查和原生表单已实现。提醒取消/列表/到期、投票参与/关闭/附图/截止/实时结果待实现 |
...
7. 使用专用测试账号和虚构数据验收首次单聊、附件送达和投票创建；随后分析附件二进制响应、投票参与、实时同步和加密会话。
