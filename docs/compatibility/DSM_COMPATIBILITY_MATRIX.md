...
| 历史消息 | `SYNO.Chat.Post.list` | 已接线 offset/limit 向上分页、消息 ID 去重、原阅读位置恢复和前台每 15 秒轻量增量刷新；空发送者名称由用户目录回填 | 复验发送者、顺序、总数、跨页重复、增量遗漏和消息时间单位 |
...
| 附件读取/缩略图 | `SYNO.Chat.Post.File.get(post_id)`、`thumbnail(post_id,type)` v2（内部接口） | 参数已由官方网页客户端静态确认；岚仓已加入能力发现，下载、缩略图和打开尚未实现 | 确认响应类型、Range、权限、缓存和离线行为后再开放 |
...
| 提醒 | `SYNO.Chat.Post.Reminder.set/list/delete/get` v1 | `set(post_id,remind_at)` 与 `delete(post_id)` 已确认；岚仓目前仅接入设置和进程内去重 | 时间单位、读取、修改、取消和到期行为 |
| 投票 | `SYNO.Chat.Post.Vote.create/close/delete/set/get_choices/vote/create_option` v1 | 已接入无附件投票创建、输入校验、重复提交保护、结果回读和原生创建表单；参与、关闭、删除、附图和结果实时同步未实现 | 用虚构数据验证创建返回、截止时间单位、单选/多选、匿名、权限、投票与关闭 |
...
| 实时增量 | `sc/socket.io`（内部协议） | 官方客户端的初始化、消息/频道/输入状态事件及重连策略已确认；岚仓仍使用 15 秒前台轻量刷新 | 在独立适配器完成事件去重、断线重连、会话撤销和后台策略后再接入 |
