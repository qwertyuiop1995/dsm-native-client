...
| Virtual Machine Manager | 2.6.5-12202 | 官方网页可见 + `SYNO.API.Info` + Synology 官方 VMM API 指南 + 2026-07-27 已登录页面只读导航、创建/修改请求发送前拦截、日志页面与前端读取契约核对和 noVNC 地址生成逻辑核对 | macOS 已接入官方 `SYNO.Virtualization.API.*` v1 优先和内部接口隔离降级；Android 第 78 批按公开 Guest v1 `additional=true` 增加磁盘/网卡只读详情，按 Task.Info v1 增加最多 100 项的只读任务中心；第 79 批使用官方 `Guest.Image.create` v1 实现从 NAS 已有文件创建映像，具备源文件/存储/名称基线、同名防重复、单次提交、`Task.Info.get` 跟踪、稳定 `image_id` + 名称 + 类型严格回读、终态 `clear`，断线和取消不重放。任务 ID 仅保存在 Workspace，可跨 Activity 配置重建继续核对 | 第 78、79 批仅增加公开指南驱动的合成契约/JVM 测试与 API 35 模拟器界面证据，不升级当前真实环境证据等级；真实 NAS 的映像格式、文件权限、存储状态、任务字段与写入结果待用户打包验证。应用进程死亡或重启后的任务 ID 恢复未实现，需另行授权持久结构；映像上传、编辑、导出以及迁移、克隆仍未完成，内部网络修改/删除仍需专用目标验证 |
...
| Download Station | 4.1.2-5012 | 官方公开指南 + 2026-07-27 已登录页面只读导航与脱敏字段核对 + 2026-07-29 套件中心版本复核 | macOS 官方 `SYNO.DownloadStation.*` 优先，当前 NAS 的 `SYNO.DownloadStation2.*` 独立降级；Android 已接入 Tracker/Peer、BT 搜索、RSS 浏览/单站点刷新和结构化任务控制，第 78 批新增官方 Task.edit v1 单任务保存位置：任务/可写目录双基线、单次提交、严格回读、断线/取消不重放及持久反馈，不回退到 DownloadStation2 | Android 第 78 批只增加公开指南驱动的合成写入/回读和 API 35 界面证据，不升级真实写行为等级；真实文件移动副作用、权限、断线与版本差异待用户打包验收。官方指南仍未公开 RSS 完整编辑或文件优先级写参数；高级 BT、监听目录、NZB、RSS 与通知设置仍需契约和专用目标验收 |
