...
| Android 客户端 | 已实现，需设备验收 | 第 84 批收紧 VMM Task.Info 轮询为仅任务分区可见且有未结束任务时运行，并增加 VMM 任务、NAS 性能两个严格无载荷固定深页；第 83 批的套件更新提示、真实套件图标与 Registry 官方来源标识保持有效。套件安装/升级、Container 写入与 VMM 高级写入继续关闭。A0–A8 当前仍为 183/202（90.6%），剩余 19 项；原目标没有删除、拆分或降级。实体机、TalkBack、OEM、真实进程死亡、系统文件授权和真实 NAS 权限/字段/副作用保持未验证。 |
