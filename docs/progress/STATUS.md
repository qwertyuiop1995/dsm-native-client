...
| 请求契约与统一写操作结果 | RC0/MR0 已实现，RC1 第十四批和 MR2 互联网代理设置批次已测试 | 请求 Fixture Schema、敏感参数仅记录存在、隐私/危险重试门禁和 CI 已接入；Apple 已对照 File Station、账号/群组、套件、容器/VMM、Download Station、QuickConnect、物理网卡、S.M.A.R.T. 检测、安全、硬件、远程访问、文件服务、远程终端及互联网代理设置三十八份请求快照；收藏新增、File Station 批量删除、套件卸载、账号/群组删除、物理网卡、安全设置、S.M.A.R.T. 检测启停、硬件、远程访问、文件服务、远程终端、互联网代理及服务管理删除已贯通统一结果语义，代理保存完成地址/端口/能力预检、防重复、逐字段回读，并区分确认成功、部分成功、提交未确认与取消，Android 类型测试通过，Windows 类型与测试源码等待 Windows CI；共享权限、更新及其余写操作继续迁移 |
...
| 桌面云盘位置 | `IMPLEMENTED`、`UNIT_TESTED`、`BUILD_VERIFIED`、`SIGNING_REQUIRED` | 双平台只读源码已完成：按需读取、文件/目录离线保留、递归空间预检、分项缓存、LRU、缓存卷、后台驻留和恢复均已接入；macOS 创建/移除已接入可恢复事务，覆盖启动续清理、系统盘 domain 补注册、外接卷保护和孤立 domain 清理；主 App 继续使用应用内 AES-GCM，只有存在映射时才共享不含密码的最小会话；临时签名包能力降级和 446+4 项测试通过，仍需正式签名 Finder、App Group、共享 Keychain 与真实 NAS 验收 |
...
| NAS 设置模块 | 需要验证 | 已接入存储、套件、任务、账号、日志、连接、文件服务、远程终端、代理、物理网卡、DDNS、区域时间、远程访问、防火墙基础控制、安全防护、局域网发现、风扇/提示音、休眠节能及 UPS；套件卸载、账号/群组删除、物理网卡、安全设置、S.M.A.R.T. 检测启停、硬件、远程访问、文件服务、远程终端和互联网代理设置保存已区分确认成功、明确拒绝、部分成功、提交未确认与取消，未知结果会要求重新连接或读取全部相关设置后核对，且不自动重放；其余新增写操作均确认、防重复并回读，等待专用测试目标实机验收 |
...
- 本地化检查覆盖 Apple、Android、Windows 的双语键、格式参数、资源引用、英语资源残留中文和生产界面硬编码；当前统计为 Apple 2,399 个、Android 246 个、Windows 262 个双语资源键。
- Apple 共享包 446 项 XCTest 通过（2 项按条件跳过），4 项 Swift Testing 通过，`DsmMac` Debug 无签名构建和临时签名 Release 打包启动通过；Android 23 项单元测试与 Debug APK 构建通过。Windows 既有 38 项测试结果保持有效，本批新增 3 项结果模型测试源码因当前 macOS 环境缺少 .NET 10 SDK，等待 Windows CI 编译执行。
