...
- 系统文件选择器上传、保存面板下载和统一传输中心。
- 带确认、任务轮询和结果校验的删除。
- 回收站浏览与受兼容开关保护的恢复到原位置。
- 不访问网络的完整演示模式。

密码和 OTP 只保留在登录界面的内存状态中；成功或非 OTP 错误后立即清空。回收站恢复必须先在目标 DSM build 上完成实机验证。

上传和下载由工作区统一管理，离开“传输中心”后仍会继续；传输中心会显示进度、速度和预计剩余时间。当前版本要求应用保持运行，退出应用会取消尚未完成的任务。
