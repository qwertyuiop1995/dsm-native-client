...
| Virtual Machine Manager | 2.6.5-12202 | 官方网页可见 + `SYNO.API.Info` + Synology 官方 VMM API 指南 + 2026-07-27 已登录页面只读导航、创建/修改请求发送前拦截、日志页面与前端读取契约核对和 noVNC 地址生成逻辑核对 | macOS 已接入官方 `SYNO.Virtualization.API.*` v1 优先和内部接口隔离降级；Android 第 78 批按公开 Guest v1 `additional=true` 增加磁盘/网卡只读详情，按 Task.Info v1 增加最多 100 项的任务中心；任务列表只向界面提供单向摘要稳定键、结束状态和进度，真实任务 token 仅保留在当前 Workspace 内存与请求边界。用户明确确认数量后，清理流程以全量 `list/get` 重新核对基线，只逐项 `clear` 已结束且身份未漂移的任务，进行中任务不受影响；提交异常或取消只执行一次严格回读，不自动重放，并允许报告部分完成或结果未知。第 79 批另使用官方 `Guest.Image.create` v1 实现从 NAS 已有文件创建映像，具备源文件/存储/名称基线、同名防重复、单次提交、`Task.Info.get` 跟踪、稳定 `image_id` + 名称 + 类型严格回读、终态 `clear`，断线和取消不重放。映像任务 ID 仅保存在 Workspace，可跨 Activity 配置重建继续核对 | 新增能力仅增加公开指南驱动的合成契约、JVM 与模拟器界面证据，不升级当前真实环境证据等级；真实 NAS 的任务字段、清理权限、`clear` 副作用、取消与部分完成边界，以及映像格式、文件权限、存储状态和写入结果待用户打包验证。应用进程死亡或重启后不能恢复清理基线、真实任务 token 或映像任务 ID；高级磁盘/网络、迁移、克隆、映像上传、编辑、导出仍未完成，内部网络修改/删除仍需专用目标验证 |
...
| Download Station | 4.1.2-5012 | 官方公开指南 + 2026-07-27 已登录页面只读导航与脱敏字段核对 + 2026-07-29 套件中心版本复核 | macOS 官方 `SYNO.DownloadStation.*` 优先，当前 NAS 的 `SYNO.DownloadStation2.*` 独立降级；Android 已接入 Tracker/Peer、RSS 浏览/单站点刷新和结构化任务控制；BT 搜索使用官方 `SYNO.DownloadStation.BTSearch` v1 读取提供方与类别，支持全部、仅启用或指定提供方范围，以及类别、标题、排序字段和方向，不回退到 `DownloadStation2`；当前活动摘要使用官方 `SYNO.DownloadStation.Statistic.getinfo` v1，独立显示标准任务与 eMule 的当前聚合上传/下载速率，读取失败不替换任务列表。第 78 批另新增官方 Task.edit v1 单任务保存位置：任务/可写目录双基线、单次提交、严格回读、断线/取消不重放及持久反馈 | 新增能力只增加公开指南驱动的合成请求、领域与模拟器界面证据，不升级真实环境或写行为等级；真实 NAS 的搜索提供方/类别、速率字段、权限与版本差异，以及文件移动副作用和断线边界待用户打包验收。官方指南仍未公开 RSS 完整编辑或文件优先级写参数；BT 协议高级设置、监听目录、NZB、RSS 与通知设置仍需契约和专用目标验收 |
