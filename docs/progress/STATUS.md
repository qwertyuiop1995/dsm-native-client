...
| Android 客户端 | 已实现，需设备验收 | 第 82 批增加唯一固定无载荷深页 `lanstash://open/containers/registry`，待处理导航只保存枚举并覆盖未就绪等待、Activity 重建、最新请求优先、模块/能力门禁、确定消费与既有返回栈；VMM 删除映像 Fixture 已从错误的任务轮询改为资源列表回读策略。审计确认 Container Compose/部署与 VMM 高级写入仍无行为验证，继续关闭而不猜接口。A0–A8 当前仍为 183/202（90.6%），剩余 19 项；原目标没有删除、拆分或降级。实体机、TalkBack、OEM、真实进程死亡、系统文件授权和真实 NAS 权限/字段/副作用保持未验证。 |
